...
class Game {
  ...
  renderLevelMessage(ctx) {
    let lvlText = 'START GAME';

    ctx.fillStyle = 'white';
    ctx.textAlign = 'center';

    if (this.state === GAME_STATE.LEVEL_UP) {
      lvlText = `START LEVEL ${this.level}`;
      ...
      ctx.fillText(cityText, 400, 200);
      ctx.fillText('+', 400, 225);
      ctx.fillText(missileText, 400, 250);
    }
    // ctx.font = '36px Helvetica';
    ctx.font = '20px PressStart';
    ctx.fillText('click to', 400, 50);
    ctx.font = '36px PressStart';
    ctx.fillText(lvlText, 400, 100);
  }
  ...
  renderEndGameMessage(ctx) {
    ...
    ctx.fillText(`Final Score ${this.score.total}`, 400, 300);
    ctx.font = '20px PressStart';
    ctx.fillText('click to play again', 400, 400);
  }
